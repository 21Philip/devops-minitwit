...
namespace Chirp.API.Controllers
{
    ...
    public class MinitwitApiController : ControllerBase
    {
        ...
        private readonly ILogger<MinitwitApiController> logger;
        ...
        ///
        /// </summary>
        /// <remarks>"demo"Get list of users followed by the given user.  - Query param &#x60;?no&#x3D;&#x60; limits result count. - Optionally updates a &#39;latest&#39; global value via &#x60;?latest&#x3D;&#x60; query param.</remarks>
        ...
        {
            this.logger.LogInformation("Fetching follows for user {Username} with limit {Limit}", username, no ?? int.MaxValue);
...
#pragma warning restore SA1010, SA1003
        ...
        {
            this.logger.LogInformation("Fetching public API messages with limit {Limit}", no ?? int.MaxValue);
        ...
        {
            this.logger.LogInformation("Fetching API messages for user {Username} with limit {Limit}", username, no ?? int.MaxValue);
        ...
        public virtual async Task<IActionResult> PostFollow([FromRoute(Name = "username")][Required] string username, [FromBody] FollowAction payload, [FromQuery(Name = "latest")] int? latest)
        {
            ...
            if (payload.Follow is string followeeName)
            {
                ...
                this.logger.LogInformation("API user {Username} followed {FolloweeName}", username, followeeName);
            ...
            if (payload.Unfollow is string unfolloweeName)
            {
                ...
                this.logger.LogInformation("API user {Username} unfollowed {UnfolloweeName}", username, unfolloweeName);
                return this.NoContent();
            }
        ...
        public virtual async Task<IActionResult> PostMessagesPerUser([FromRoute(Name = "username")][Required] string username, [FromBody] PostMessage payload, [FromQuery(Name = "latest")] int? latest)
        {
            ...
            this.logger.LogInformation("API user {Username} created cheep successfully", username);
            return this.NoContent();
        }
        ...
        {
            this.logger.LogInformation("Processing API registration for username {Username}", payload?.Username);
            ...
            {
                this.logger.LogInformation("API registration succeeded for username {Username}", payload.Username);
                return this.NoContent();
            }

            this.logger.LogWarning("API registration failed for username {Username}", payload?.Username);
            return this.BadRequest();
        }
        ...
        {
            this.logger.LogInformation("Processing API delete request for user {Username}", username);
            ...
            {
                this.logger.LogInformation("API delete succeeded for user {Username}", username);
                return this.NoContent();
            }

            this.logger.LogWarning("API delete failed for user {Username}", username);
            return this.BadRequest();
        }
    }
}
